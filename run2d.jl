...
lambda_sigma = [1.1, 0.25]  #lambda  = smoothness penalty coeff and sigma =  the scale of the reproducing kernel

# generate the data:  X
tmpx = [rand(50), randn(30)/10 + .8]
...
x_grd, y_grd =  meshgrid(linspace(-2.5, 2.5, 150),linspace(-2.5, 2.5, 150))   
N_grd = length(x_grd)
...
# target = targetNormal2d # this sets an alias to a function giving in the file targets.jl
